...
export const FileDir: React.FC<FileComponentProps> = ({ file, depth }) => {
  ...
  return (
    <Collapse.def key={file.name} className={"relative"}>
      <Collapse.Trigger
        title={file.path}
        className={`bg-zinc-900 px-1 text-sm cursor-pointer hover:bg-black/70 transition-all flex flex-nowrap justify-between items-center gap-x-1 py-1 w-full group before:absolute before:left-${depth} before:top-0 before:min-w-[1px] before:h-full before:bg-zinc-500`}
